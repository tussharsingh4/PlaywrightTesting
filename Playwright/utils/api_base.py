...
class APIUtils: #this utility is going to create an order using the calls to the api.
    ...
    def create_order(self, playwright:Playwright, user_credentials): #even when doing api or ui automation.

        token = self.get_token(playwright, user_credentials) #we have to get the token first before creating the order. because we need the token for authentication. so we can call the get_token method here to get the token and then use it for creating the order.
